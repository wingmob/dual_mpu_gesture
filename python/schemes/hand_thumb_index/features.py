...
SENSOR_PREFIXES = ["hand", "thumb", "index"]
PAIR_PREFIXES = ["hand_thumb", "hand_index", "thumb_index"]
SENSOR_SIGNAL_SUFFIXES = [
    "ax_g",
    "ay_g",
    "az_g",
    "gx_dps",
...
GLOBAL_SIGNAL_COLUMNS = [
    "hand_to_digits_motion_energy",
    "digits_pair_motion_energy",
    "all_pair_motion_energy_mean",
    "all_pair_motion_energy_max",
    "mean_digit_gyro_dps",
    "mean_all_gyro_dps",
    "mean_all_accel_g",
    "motion_energy",
...
FEATURE_SIGNAL_COLUMNS = (
    [f"{sensor}_{suffix}" for sensor in SENSOR_PREFIXES for suffix in SENSOR_SIGNAL_SUFFIXES]
    + [f"{pair}_{suffix}" for pair in PAIR_PREFIXES for suffix in PAIR_SIGNAL_SUFFIXES]
    + GLOBAL_SIGNAL_COLUMNS
)

POSE_STATE_COLUMNS = (
    [f"{sensor}_{suffix}" for sensor in SENSOR_PREFIXES for suffix in ("roll_deg", "pitch_deg")]
    + [f"{pair}_{suffix}" for pair in PAIR_PREFIXES for suffix in ("rel_roll_deg", "rel_pitch_deg", "rel_yaw_deg", "gravity_angle_deg", "pose_gap_deg")]
)
...
def select_peak_window(raw_data: pd.DataFrame, window_samples: int = 180) -> pd.DataFrame:
...
def extract_feature_vector(raw_or_enriched: pd.DataFrame) -> dict[str, float]:
    ...
    features["tail_hand_to_digits_motion_energy_mean"] = _window_mean(tail["hand_to_digits_motion_energy"])
    features["tail_digits_pair_motion_energy_mean"] = _window_mean(tail["digits_pair_motion_energy"])
    features["tail_all_pair_motion_energy_mean_mean"] = _window_mean(tail["all_pair_motion_energy_mean"])
    features["tail_all_pair_motion_energy_max_mean"] = _window_mean(tail["all_pair_motion_energy_max"])
    features["tail_mean_digit_gyro_dps_mean"] = _window_mean(tail["mean_digit_gyro_dps"])
...
class MotionSegmenter:
    start_threshold_dps: float = 22.0
    stop_threshold_dps: float = 8.0
    quiet_frames: int = 16
    min_frames: int = 18
    max_frames: int = 220
    pre_roll: int = 12
