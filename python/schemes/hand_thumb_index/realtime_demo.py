...
from .pose import HandThumbIndexStreamProcessor
from ...common.triple_protocol import DEFAULT_BAUD, convert_counts_to_physical, parse_frame_line
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Realtime hand/thumb/index triple MPU6050 gesture recognition demo.")
    ...
    parser.add_argument("--start-threshold", type=float, default=22.0, help="Motion energy threshold to start a segment.")
    parser.add_argument("--stop-threshold", type=float, default=8.0, help="Motion energy threshold to count quiet frames.")
    parser.add_argument("--quiet-frames", type=int, default=16, help="How many quiet frames end a segment.")
    parser.add_argument("--min-frames", type=int, default=18, help="Minimum segment length to classify.")
    parser.add_argument("--max-frames", type=int, default=220, help="Maximum segment length before forced classification.")
    parser.add_argument("--min-duration", type=float, default=0.20, help="Ignore segments shorter than this duration in seconds.")
    parser.add_argument("--min-peak", type=float, default=40.0, help="Ignore segments whose motion peak is below this value.")
...
def render_dashboard(
    ...
    lines = [
        "Hand + Thumb + Index Triple MPU Gesture Realtime",
        f"Port: {port} @ {baud}",
...
def wait_for_stream_ready(ser: serial.Serial, timeout_s: float = 14.0) -> dict[str, int] | None:
    print("Waiting for board reset and gyro calibration (~8s)...")
